...
module HarzardUnit(
    ...
    input wire PredictedE, //BTB
    input wire PredictedTakenE,//BHT
    ...
    always @ (*)
        ...
        else if(JalrE || (PredictedE && (BranchE ^ PredictedTakenE)) || (~PredictedE && BranchE))
            //Jalr和Branch在EX段结束，需要刷新FlushD和FlushE
            //进行预测但预测结果和实际跳转结果不一致
            //没有预测但实际跳转了
            {StallF,FlushF,StallD,FlushD,StallE,FlushE,StallM,FlushM,StallW,FlushW} <= 10'b0001010000;
        else if(MemToRegE & ((RdE==Rs1D)||(RdE==Rs2D)) )//当EX段执行的是Load指令时，且写回的寄存器会被ID段代码用到，则需要插入气泡停顿一个周期
    ...
    //对寄存器1的定向路径。处理的情况是EX段计算时需要用到Mem或WB段的数据；并且注意0号寄存器的特殊性
    always@(*)begin
