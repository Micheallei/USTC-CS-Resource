from process_data import load_and_process_data,standardization
...
class LinearClassification:
    ...
    def fit(self,train_features,train_labels):
        ...
        需要你实现的部分
        '''
        self.W=np.random.random_sample(size = (8, 1)) #初始化参数
        for i in range(0,self.epochs):
            gradient=2/train_features.shape[0]*np.dot(train_features.transpose(),np.dot(train_features,self.W)-train_labels) + 2*self.Lambda*self.W
    ...
    def predict(self,test_features):
        ...
        需要你实现的部分
        '''
        pred = np.dot(test_features,self.W)
        return np.around(pred).astype(int)#预测值需要取整
